...
class _MyMapsState extends State<MyMaps> {
  ...
  Future<void> _showMyDialog(name, id) async {
    return showDialog<void>(
      context: context,
      barrierDismissible: false, // user must tap button!
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          actions: <Widget>[
            TextButton(
              ...
              onPressed: () async {
                Database database =
                    await openDatabase('vanishedTrails.db ', version: 1);
    ...
    for (int i = 0; i < _list.length; i++) {
      ...
      points.add(Dismissible(
          ...
          onDismissed: (DismissDirection direction) async {
            print(direction);
            _showMyDialog(_list[i]['name'], _list[i]['id']);
          },
          child: Container(
              margin: EdgeInsets.only(top: 10),
                      ...
                      child: OutlinedButton(
                          onPressed: () async {
                            ...
                            if (connectivityResult == ConnectivityResult.none) {
                              ...
                              Navigator.of(context).pushNamed('/uploadmap',
                                  arguments: _list[i]['name']);
                            }
                          },
                          child: Row(children: [
